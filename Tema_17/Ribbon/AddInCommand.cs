...
namespace Ribbon
{
    ...
    public class CreateWall : IExternalCommand, IExternalCommandAvailability
    {
         public bool IsCommandAvailable(Autodesk.Revit.UI.UIApplication applicationData, CategorySet selectedCategories)
        {
            Level newWallLevel = GetNewWallLevel(applicationData); //Obtenemos Level de Combobox - LevelsSelector
                                                                        //Comparamos Level
            if (newWallLevel.Name != applicationData.ActiveUIDocument.ActiveView.Name) return false;
            ...
            {
                if (c.Id.IntegerValue == (int)BuiltInCategory.OST_Walls) return true;
            }
            return false;
        }
        ...
        {
#if NewTab
            RibbonPanel myPanel = app.GetRibbonPanels("Revit API Manual").Where(x => x.Name == "Ribbon Sample").FirstOrDefault();
#else
            RibbonPanel myPanel= app.GetRibbonPanels().Where(x => x.Name == "Ribbon Sample").FirstOrDefault(); 
#endif
    ...
    public class ResetSetting : IExternalCommand
    {
        ...
        {
#if NewTab
            RibbonPanel myPanel = revit.Application.GetRibbonPanels("Revit API Manual").Where(x => x.Name == "Ribbon Sample").FirstOrDefault();
#else
            RibbonPanel myPanel = revit.Application.GetRibbonPanels().Where(x => x.Name == "Ribbon Sample").FirstOrDefault(); 
#endif
            //reset wall type
            RadioButtonGroup radioGroupTypeSelector =
                GetRibbonItemByName(myPanel, "WallTypeSelector") as RadioButtonGroup;
