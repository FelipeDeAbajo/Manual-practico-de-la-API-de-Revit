...
namespace Unidades
{
    ...
    public class UnidadesPre2021 : IExternalCommand
    {
          ...
          ElementSet elements)
        {
            ...
            else if (doc.GetElement(elementIdsList.FirstOrDefault()) is Wall wall)
            {
                ...
                using (Transaction tx = new Transaction(doc))
                {
                    ...
                    //multiplicamos altura * 1.15 
                    double nuevaAlturaMetros = alturaInicialMetros * 2.15;
                    //Convertir a unidades internas. Concertimos 10 metros
                    double nuevaAlturaInterna = UnitUtils.ConvertToInternalUnits(nuevaAlturaMetros
                        , /*UnitTypeId.Meters*/ DisplayUnitType.DUT_METERS);
                    //Actualizamos el parámetro del muro con el nuevo valor (10 metros)
                ...
                bool parsed = UnitFormatUtils.TryParse(units, /*SpecTypeId.Length*/ UnitType.UT_Length, alturaTxtMetros
                    , valueParsingOptions, out double valorConvertidoDesdeString, out message);

                if (parsed == false)
                {
                    message = "Introducir tecto correcto en metros";
                    return Result.Failed;
                }
                ...
                #region Convertir un numero Revit a string a)
